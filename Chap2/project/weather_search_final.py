import requests
import json
#from pprint import pprint # pprint 模块用于打印json数据，以整理好的形式打印，也可以用快捷键格式化数据
...
def fetchWeather(input_words):
    ...
    #pprint(r)
    city = r['results'][0]['location']['name'] #list 只有一个，取出list用下标，字典用key提取value
...
while True:
    input_words = input("请输入您要查询的城市或指令，如'北京'或'beijing',如需帮助，请输入help。\n> ")
    if input_words == "help":
        print("""
            输入'help'，打印帮助文档；
            输入'exit'，退出程序
